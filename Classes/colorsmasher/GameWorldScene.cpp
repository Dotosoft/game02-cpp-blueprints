...
void GameWorld::calculateScreensize()
{
	...
	auto tileWidth = Sprite::create(RESOURCES_COLOR_IMAGE_TILE)->getContentSize().width;
...
void GameWorld::createBackground()
{
	...
	auto titleLabel = Label::createWithTTF("ColourSmash", RESOURCES_COLOR_FONT_COMIC_SANS, 32);
	...
	auto pauseButtonNormal = Sprite::create(RESOURCES_COLOR_IMAGE_BUTTON_PAUSE);
	auto pauseButtonClicked = Sprite::create(RESOURCES_COLOR_IMAGE_BUTTON_PAUSE);
...
void GameWorld::createTileSprites()
{
	...
	this->spriteBatchNode = SpriteBatchNode::create(RESOURCES_COLOR_IMAGE_TILE, NUM_COLS * NUM_ROWS + NUM_ROWS);
...
void GameWorld::createTileSprite(int tileId)
{
	...
	auto tileTmp = Sprite::create(RESOURCES_COLOR_IMAGE_TILE);
...
void GameWorld::createHUD()
{
	...
	this->scoreLabel = Label::createWithTTF(__String::createWithFormat("Score: %d", this->score)->getCString(), RESOURCES_COLOR_FONT_COMIC_SANS, 18);
	this->scoreLabel->setPosition(Point(this->screenSize.width * 0.33f, this->screenSize.height * 0.875f));
	this->addChild(this->scoreLabel, 1);

	this->timeLabel = Label::createWithTTF(__String::createWithFormat("Time: %d", this->time)->getCString(), RESOURCES_COLOR_FONT_COMIC_SANS, 18);
...
void GameWorld::doCountdownAnimation()
{
	...
	for (int i = 0; i<4;i++)
	{
		auto label = Label::createWithTTF("", RESOURCES_COLOR_FONT_COMIC_SANS, 52);
...
void GameWorld::showScoreText(int scoreToAdd, Vec2 point) {
	...
	auto bonusLabel = Label::createWithTTF(__String::createWithFormat("+%d", scoreToAdd)->getCString(), RESOURCES_COLOR_FONT_COMIC_SANS, 32);
...
void GameWorld::showPausePopup() {
	...
	auto continueButton = MenuItemLabel::create(Label::createWithTTF("Continue", RESOURCES_COLOR_FONT_COMIC_SANS, 32), this, menu_selector(GameWorld::onContinueClicked));
	continueButton->setPosition(Vec2(this->screenSize.width*0.5f, this->screenSize.height*0.6f));

	// create the restart button
	auto restartButton = MenuItemLabel::create(Label::createWithTTF("Restart", RESOURCES_COLOR_FONT_COMIC_SANS, 32), this, menu_selector(GameWorld::onRestartClicked));
	restartButton->setPosition(Vec2(this->screenSize.width*0.5f, this->screenSize.height*0.5f));

	// create the menu button
	auto menuButton = MenuItemLabel::create(Label::createWithTTF("Menu", RESOURCES_COLOR_FONT_COMIC_SANS, 32), this, menu_selector(GameWorld::onMenuClicked));
	...
	auto pausedLabel = Label::createWithTTF("Game Paused", RESOURCES_COLOR_FONT_COMIC_SANS, 52);
...
void GameWorld::showGameOverPopup() {
	...
	// title to inform the user which popup this is
	auto gameOverLabel = Label::createWithTTF("Game Over", RESOURCES_COLOR_FONT_COMIC_SANS, 52);
	...
	auto scoreLabel = Label::createWithTTF(__String::createWithFormat("Score: %d", this->score)->getCString(), RESOURCES_COLOR_FONT_COMIC_SANS, 48);
...
void GameWorld::drawDebugScreen(Vec2 *vertices) {
	...
	for (int i = 0; i<lengthOfVertice; i++) {
		...
		auto debugLabel = Label::createWithTTF(__String::createWithFormat("%d", i)->getCString(), RESOURCES_COLOR_FONT_COMIC_SANS, 24);
