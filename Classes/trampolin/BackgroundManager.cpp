...
bool BackgroundManager::init()
{
	...
	num_bg_pieces_ = 0;
	bg_sprite_ = NULL;
	bg_piece_size_ = Size::ZERO;

	// create & add the batch node
	batch_node_ = SpriteBatchNode::create("cjtexset_02.png", 20);
	addChild(batch_node_);

	// create an array to maintain pieces of the background
	bg_pieces_ = __Array::create();
	bg_pieces_->retain();
	CreateBG();

	// create an array to maintain pieces of the walls
	wall_sprites_ = __Array::createWithCapacity(4);
...
void BackgroundManager::CreateBG()
{
	...
	bg_sprite_ = Sprite::createWithSpriteFrameName("cjbg01.png");
	bg_sprite_->setPosition(Vec2(SCREEN_SIZE.width/2, SCREEN_SIZE.height/2));
	batch_node_->addChild(bg_sprite_);

	// fill up a screen with the loopable bg pieces
	Sprite* bg_piece = Sprite::createWithSpriteFrameName("cjbg02.png");
	bg_piece_size_ = bg_piece->getContentSize();
	bg_piece->setPosition(Vec2(SCREEN_SIZE.width/2, bg_sprite_->getPositionY() + bg_sprite_->getContentSize().height/2 + bg_piece_size_.height/2));
	...
	{
		bg_piece = Sprite::createWithSpriteFrameName("cjbg02.png");
		bg_piece->setPosition(Vec2(SCREEN_SIZE.width/2, position_y + bg_piece_size_.height));
		bg_pieces_->addObject(bg_piece);
		batch_node_->addChild(bg_piece);
...
void BackgroundManager::CreateWall()
{
	...
	Texture2D* texture = Director::getInstance()->getTextureCache()->addImage("cjsbar01.png");
	// ccTexParams texture_param;
	Texture2D::TexParams texture_param;
	...
	for(int i = 0; i < 4; ++i)
	{
		Sprite* wall_sprite = Sprite::createWithTexture(texture, Rect(0, 0, WALL_WIDTH, SCREEN_SIZE.height));
...
void BackgroundManager::UpdateBG(float speed)
{
	...
	{
		Sprite* bg_piece = (Sprite*)bg_pieces_->getObjectAtIndex(i);
...
void BackgroundManager::UpdateWall(float speed)
{
	...
	for(int i = 0; i < 4; ++i)
	{
		Sprite* wall_sprite = (Sprite*)wall_sprites_->getObjectAtIndex(i);
	...
	for(int i = 0; i < 2; ++i)
	{
		...
		Sprite* wall_sprite1 = (Sprite*)wall_sprites_->getObjectAtIndex(i*2);
		Sprite* wall_sprite2 = (Sprite*)wall_sprites_->getObjectAtIndex(i*2 + 1);
