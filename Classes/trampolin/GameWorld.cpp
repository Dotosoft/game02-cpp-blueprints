...
GameWorld::GameWorld()
{
	...
	touch_start_ = Point::ZERO;
	touch_end_ = Point::ZERO;
	score_label_ = NULL;
	score_ = 0;
...
void GameWorld::update(float dt)
{
	...
	if(platform_->isVisible() && !touch_start_.equals(Point::ZERO))
	{
...
void GameWorld::onTouchEnded(const std::vector<Touch*>& touches, Event* evt)
{
	...
	touch_start_ = touch_end_ = Point::ZERO;
}
...
void GameWorld::onAcceleration(Acceleration* acc, Event* event)
{
	...
	clown_->GetBody()->SetLinearVelocity(b2Vec2(acc->x * 10, 10.0f));
}
...
void GameWorld::DoCollisionResponse(float dt)
{
	...
	clown_->GetBody()->ApplyLinearImpulse(impulse, clown_->GetBody()->GetWorldCenter(), true);
