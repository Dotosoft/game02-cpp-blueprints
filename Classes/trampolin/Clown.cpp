...
void Clown::Update()
{
	...
	highest_position_ = getPosition().y > highest_position_ ? getPosition().y : highest_position_;

	// if clown has moved two screens lower than highest point, its game over
	if(highest_position_ - getPosition().y > SCREEN_SIZE.height * 2)
	{
		game_world_->GameOver();
	}
	
	// update rocket jet stream if it exists
	if(rocket_trail_) rocket_trail_->setPosition(getPosition().x - getContentSize().width/3, getPosition().y);
...
void Clown::SetBody(b2Body* body)
{
	...
	clown_shape.SetAsBox(SCREEN_TO_WORLD(getContentSize().width * 0.5), SCREEN_TO_WORLD(getContentSize().height * 0.5f));
