...
bool Sky::init()
{
	...
	sprite_->setTextureRect(Rect(0, 0, (SCREEN_SIZE.width / SCREEN_SIZE.height * 1024), 1024));
	// position the sprite at the centre and add it
	sprite_->setPosition(Vec2(SCREEN_SIZE.width/2, SCREEN_SIZE.height/2));
	addChild(sprite_);

	// create some clouds
	clouds_ = Sprite::create("clouds.png");
	...
	clouds_->setTextureRect(Rect(0, 0, (SCREEN_SIZE.width / SCREEN_SIZE.height * 1024), 256));
	// position the clouds at the top of the screen and add it
	clouds_->setPosition(Vec2(SCREEN_SIZE.width/2, SCREEN_SIZE.height * 0.9f));
	addChild(clouds_);

	return true;
}

Sprite* Sky::GenerateSkySprite()
{
	float texture_size = 1024;
	// create a texture that we can draw into
	RenderTexture* render_texture = RenderTexture::create(texture_size, texture_size);
	...
	Sprite* noise = Sprite::create("noise2.png");
	...
	noise->setPosition(Vec2(texture_size/2, texture_size/2));
...
void Sky::Update(Point penguin_position, float terrain_offset_x, float terrain_scale)
{
	// update the offset
	...
	if (offset_x_ != offset_x)
	{
		...
		sprite_->setTextureRect(Rect(offset_x_, 0, sprite_->getTextureRect().size.width, sprite_->getTextureRect().size.height));
		clouds_->setTextureRect(Rect(offset_x_ * 2, 0, clouds_->getTextureRect().size.width, clouds_->getTextureRect().size.height));
	}
